...
int update_acro_rec(int update_rec_id)
{
        ...
        rc = sqlite3_prepare_v2(
            db, "select rowid, ifnull(Acronym,''),"
        ...
        if (update_rec_count == 1) {
                ...
                if (strcasecmp((const char *)cont_del, "y") == 0) {
                        ...
                        while (1) {
                                ...
                                add_history(u_acro);
                                u_acro_expd =
                                    readline("Enter the expanded acronym: ");
                                add_history(u_acro_expd);
                                u_acro_desc = readline(
                                    "Enter the acronym description: \n\n");
                                ...
                                complete =
                                    readline("Enter record? [ y/n or q ] : ");
                                if (strcasecmp((const char *)complete, "y") ==
                                    0) {
                                        break;
                                }
                                if (strcasecmp((const char *)complete, "q") ==
                                    0) {
                                        /* Clean up readline allocated memory */
                                        if (complete != NULL) {
                                                free(complete);
                        ...
                        sql_update = sqlite3_mprintf(
                            "update ACRONYMS set Acronym=%Q, "
